...
def generate_actor(tenant, user, role_type_string, name=None, title=None, department=None):
    ...
    if role_type_string=='provider':
        ...
        if doc_nurse > .33:
            actor_doc = ProviderActor(provider_title=title, facility_name=department, notes=lorem.paragraph())
        else:
            actor_doc = ProviderActor(provider_title=title, facility_name=department, specialty=random_text(), notes=lorem.paragraph())
    elif role_type_string=='caregiver':
        actor_doc = CaregiverActor(notes=lorem.paragraph(), relation= random.choice(CaregiverActor.RELATIONSHIP_CHOICES)[0])
    elif role_type_string=='patient':
        actor_doc = PatientActor(notes="Patient Actor autogenerated")
