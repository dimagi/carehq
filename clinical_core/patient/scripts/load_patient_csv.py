...
def run():    
    ...
    patients = []
    structure = (
        ('dot', 7, ('sunday', 'monday', 'tuesday', 'wednesday', 'thursday', 'friday', 'saturday')),
        ('phone', 3, ('type', 'value')),
    ...
    for row in rows:
        ...
        for key in map(normalize, ('PACTID','Given Name','Family Name','DOB','Gender','Arm','HP','ART Regimen','Non-ART Regimen')):                    
    ...
    for patient in patients:        
        ...
        sex = patient['gender'].lower()[0]
        dob = patient['dob'] #%m/%d/%Y
        ...
        try:
            pt_model = Patient.objects.get(user=user)
        except:
            pt_model = Patient(user=user)
            pt_model.sex = sex
